...
open class CardListItemsAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is MyViewHolder) {
            ...
            holder.itemView.findViewById<TextView>(R.id.tv_card_name).let { it.text = model.name }
            ...
            if ((context as TaskListActivity).mAssignedMembersDetailList.size > 0) {
                ...
                for (i in context.mAssignedMembersDetailList.indices) {
                    for (memberId in model.assignedTo) {
                        if (context.mAssignedMembersDetailList[i].id == memberId) {
                            val selectedMember = SelectedMember(context.mAssignedMembersDetailList[i].id, context.mAssignedMembersDetailList[i].image)
